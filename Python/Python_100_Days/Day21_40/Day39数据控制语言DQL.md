# DQL
### 定义
DQL（数据查询语言）主要用于从数据库的二维表中检索数据，通过SELECT语句实现对数据的查询操作，可根据需求筛选、排序、分组、连接多个表的数据，是 SQL 中最常用的操作之一。
...
-- 给字段起别名（AS可省略）
SELECT `word_1` AS new_word_1, `word_2` AS new_word_2 FROM `table_name`;

-- 去除重复数据
SELECT DISTINCT `word` FROM `table_name`;
```

### 2. 条件查询(WHERE)
```sql
-- 带条件的查询
SELECT `word_1` FROM `table_name` WHERE `condition` = 'True';

-- 比较运算符：=、>、<、>=、<=、!=、<>
SELECT `word_1` FROM `table` WHERE `value` = '1000';

-- 范围查询：BETWEEN...AND
SELECT `word_1`, `word_2` FROM table_name WHERE `word_1` BETWEEN 1 AND 2;

-- 列表查询：IN
SELECT `word_1`, `word_2` FROM `table_name` WHERE `word_1` IN (1, 2, 3);

-- 空值查询
SELECT `word_1`, `word_2` FROM `table_name` WHERE `word_1` IS NULL;

-- 模糊查询：LIKE（%匹配任意字符，_匹配单个字符）
SELECT `word_1` FROM `table_name` WHERE `word_1` LIKE '张%'; -- 查找张x
```

### 3. 排序查询(ORDER BY)
```sql
-- 按指定字段排序（ASC升序、DESC降序）
SELECT `word_1`, `word_2` FROM `table_name` ORDER BY `word_1` [ASC|DESC];

-- 多字段排序
SELECT `word_1`, `word_2` FROM `table_name` ORDER BY `word_1` ASC, `word_2` DESC;
```
...
-- COUNT()；统计总数
SELECT COUNT(*) AS 所有 FROM `table_name`;

-- SUM()：求和
SELECT SUM(`col`) AS 求和 FROM `table_name`;

-- AVG()：求平均值
SELECT AVG(`mean`) AS 平均 FROM `table_name` WHERE `col` = 1;

-- MAX()/MIN()：求最大/最小值
SELECT MAX(`col`) AS 最高 FROM `table_name`;
```

### 5. 分组查询(GROUP BY)
```sql
-- 按指定字段分组
SELECT 分组字段, 聚合函数 FROM `table_name` GROUP BY 分组字段;

-- 筛选平均>60
SELECT `col` AS 列, AVG(`score`) AS 平均分
FROM `table_name`
GROUP BY `col_1`
HAVING 平均分 > 80; -- HAVING 筛选分组结果
```

### 6. 连接查询
```sql
-- 内连接（INNER JOIN）：查询两表中匹配的记录
SELECT 表1别名.字段1, 表2别名.字段2,...
FROM 表1 AS 表1别名
INNER JOIN 表2 AS 表2别名
ON 表1别名.连接字段 = 表2别名.连接字段;
...
-- 右连接 查询右表所有记录 + 左表匹配记录
SELECT 表1.别名.字段1, 表2别名.字段2,...
FROM 表1 AS 表1别名
RIGHT JOIN 表2 AS 表2别名
ON 表1别名.连接字段 = 表2别名.连接字段;
```
