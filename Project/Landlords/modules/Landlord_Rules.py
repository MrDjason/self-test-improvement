...
class LandlordRules:
    ...
    def get_card_type(cards: List[Card]) -> Tuple[CardType, int, Optional[Card]]:
        ...
        values = sorted(list(face_counts.values()))
    ...
    # 核心优化：包含2→直接不判定为顺子，跳出当前if块，不执行后续连续性判断
        if 2 in faces:
        # 不使用continue，而是直接“不进入后续逻辑”，等价于“这不是顺子”
            pass  # 或者直接写注释，什么都不做，自然跳出顺子判断
        else:
        # 只有不含2，才处理A的连续性
            processed_faces = [14 if face == 1 else face for face in faces]
            processed_faces_sorted = sorted(processed_faces)
            if processed_faces_sorted[-1] - processed_faces_sorted[0] == card_count - 1:
...
# 顺子判断结束，继续执行其他牌型
        ...
        if card_count == 8:
            if values.count(3) == 2 and values in ([1, 1, 3, 3], [2, 3, 3]):
                ...
                if not set(sub_faces) & set(triple_faces):
        ...
        # 4带2（不变）
        if card_count == 6 and values.count(4) == 1 and values in ([1, 1, 4], [2, 4]):
    ...
    # 以下is_valid_play、can_beat等其他方法完全不变，保持你原有的代码
    @staticmethod
